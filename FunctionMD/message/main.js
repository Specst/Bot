...
const getLocationData = require('../scrape/location')
 let { UploadFileUgu, TelegraPh } = require('../scrape/uploader.js')
...
├ •︎ ${prefix}test
├ •︎ ${prefix}temp
├ •︎ ${prefix}listsection1
├ •︎ ${prefix}listsection2
╰❑

┌─── ❑「 ️DOWNLOADER 」❑
├ •︎ ${prefix}play _query_
├ •︎ ${prefix}play2 _query_
├ •︎ ${prefix}ytplay _query_
├ •︎ ${prefix}lagu _query_
├ •︎ ${prefix}musik _query_
├ •︎ ${prefix}twitter _link_
├ •︎ ${prefix}tiktokvideo _link_
├ •︎ ${prefix}tiktokaudio _link_
├ •︎ ${prefix}instagramdownload _link_
├ •︎ ${prefix}instagramdl _link_
├ •︎ ${prefix}igdownload _link_
├ •︎ ${prefix}igdl link
├ •︎ ${prefix}youtubemp4 _link_
├ •︎ ${prefix}youtubemp3 _link_
├ •︎ ${prefix}mediafire _link_
╰❑
...
├ •︎ ${prefix}antilink
├ •︎ ${prefix}afk
├ •︎ ${prefix}setnamegroup
├ •︎ ${prefix}setdesc
├ •︎ ${prefix}revoke
├ •︎ ${prefix}group <open/close>
├ •︎ ${prefix}hidetag
╰❑
...
┌─── ❑「 TEXTPRO 」❑
├ •︎ ${prefix}sci_fi _text_
├ •︎ ${prefix}blackpink
├ •︎ ${prefix}lightglow
├ •︎ ${prefix}glass
├ •︎ ${prefix}hoorror_blood
├ •︎ ${prefix}sand
├ •︎ ${prefix}sketch
├ •︎ ${prefix}magma
├ •︎ ${prefix}batman
...
┌─── ❑「 INTERNET 」❑
├ •︎ ${prefix}emojimix _emoji_
├ •︎ ${prefix}detiknews
├ •︎ ${prefix}wiki
├ •︎ ${prefix}wallpaperaccess
├ •︎ ${prefix}google
├ •︎ ${prefix}gimage
├ •︎ ${prefix}gempa
├ •︎ ${prefix}covidindo
...
├ •︎ ${prefix}cuttly
├ •︎ ${prefix}ceklokasi
├ •︎ ${prefix}luas-segitiga
├ •︎ ${prefix}kel-segitiga
├ •︎ ${prefix}luas-persegi
├ •︎ ${prefix}kel-persegi
├ •︎ ${prefix}kuadrat
├ •︎ ${prefix}kubik
├ •︎ ${prefix}perkalian
╰❑
...
let fkontak = { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: '6283141953754-1604595598@g.us' } : {})}, message: { "contactMessage":{"displayName": `Specstor MD`,"vcard":`BEGIN:VCARD\nVERSION:3.0\nN:2;ryan;;;\nFN:ryan\nitem1.TEL;waid=6283141953754:6283141953754\nitem1.X-ABLabel:Mobile\nEND:VCARD` }}}               
...
break
//males pake scrape kagak dapet gw jadi pake restapi free aja :v -specstor mayan ngab
case 'githubstalk':
if (q.includes('--help')) return reply(examquery)
if (!q) return reply('*Example* : #githubstalk specst') 
if (isLimit < 1) return reply("Limit kamu sudah habis , silahkan beli dengan cara #buy limit _jumlah_")
kurangLimit(m.sender, 1)
reply(`Satu limit terpakai \nSisa limit kamu : ${getLimit(m.sender)}`)

let specsts = await fetchJson(`https:/\/\open-apis-rest.up.railway.app/api/github?username=${q}`)
let githubstalk = specsts.data
let results = githubstalk.user
let teksnya = `「 *GITHUB STALK* 」\n\n • *Nama* : ${results.name}\n • *Follower* : ${results.followers}\n • *Following* : ${results.following}\n • *Bio* : ${results.bio}\n • *Repo* : ${results.repositories}\n • *Url* : ${results.github_url}\n`
sock.sendMessage(from, { image : { url : results.avatar }, caption: teksnya}, { quoted : m } )

break
case 'igstalk':
if (q.includes('--help')) return reply(examquery)
if (!q) return reply('*Example* : #igstalk speczs__') 
if (isLimit < 1) return reply("Limit kamu sudah habis , silahkan beli dengan cara #buy limit _jumlah_")
kurangLimit(m.sender, 1)
reply(`Satu limit terpakai \nSisa limit kamu : ${getLimit(m.sender)}`) 
...
sock.sendMessage(from, { image: { url: images }, caption: '▹ Media Url : '+images }, { quoted: m })
})
}
break
...
for (let i of bc) {
sock.sendMessage(i.id, { text: `*[ SPECSTOR BROADCAST ]*\n\n${q}` })
}
break
case 'couple': {
if (q.includes('--help')) return reply(examkosong) 
...
if (q.includes('--help')) return reply(examkosong) 
 ...
 break
 case 'sell': case 'jual':{//BY LORD RIFZA
...
if (q.includes('--help')) return reply(examkosong) 
  ...
  case 'menu2':{  
   let button = [{
     index: 1, 
      urlButton: {
       ...
       url: 'https://api-specstor.herokuapp.com/api'
       } 
     },     
     {
     index: 2, 
   ...
   let buttonss = [{
       index: 1, 
       urlButton: {
       displayText: 'Portofolio Web', 
       url: 'https://waynetwork.ga/'
       }
       },
       {
       index: 1, 
    ...
    templateButtons: [
     ...
     index: 1, 
      urlButton: {
       ...
       url: 'https://github.com/specst'
      } },
     {
     index: 2, 
     callButton: {
      displayText: 'Owner', 
       phoneNumber: '6283141953754'
      } },
     {
     index: 3, 
      quickReplyButton: {
...
case 'play': case 'ytplay': case 'ytsearch': case 'yts': {
                let searchs = await yts(q)
                let anzzu = searchs.videos[Math.floor(Math.random(), searchs.videos.length)]
                let buttonzs = [
                    {buttonId: `${prefix}ytmp3 ${anzzu.url}`, buttonText: {displayText: '♫ Audio'}, type: 1},
                    {buttonId: `${prefix}ytmp4 ${anzzu.url}`, buttonText: {displayText: '► Video'}, type: 2}
                ]
                let buttonMessage = {
                    image: { url: anzzu.thumbnail },
                    caption: `
❑ Title : ${anzzu.title}
❑ Ext : Search
❑ ID : ${anzzu.videoId}
❑ Duration : ${anzzu.timestamp}
❑ Viewers : ${anzzu.views}
❑ Upload At : ${anzzu.ago}
❑ Author : ${anzzu.author.name}
❑ Channel : ${anzzu.author.url}
❑ Description : ${anzzu.description}
❑ Url : ${anzzu.url}`,
                    footer: Options.info.botName,
                    buttons: buttonzs,
                    headerType: 4
            ...
            break
   case 'play2': case 'lagu': case 'musik':
function _0x2e80(){const _0xa89407=['label','\x0a\x0a📊Duration\x20:\x20','#youtubemp4\x20','YouTube','--help','18dDgSOi','[\x20▶️\x20]\x20MP4','isYtMusic','includes','title','2494330pSwswO','130338ccTYpp','album','311080JWUDEz','470fCNgPI','1407568FUzkWN','\x0a\x0aℹ️Id\x20:\x20','lagu\x20apa?','join','log','\x0a\x0a💽Album\x20:\x20','2zjOfDc','👤Artist\x20:\x20','\x0a\x0a🔎Source\x20:\x20','reply','[\x20YouTube\x20Music\x20Search🔎\x20]','length','26382tgVgjS','url','The\x20most\x20complete\x20collection\x20of\x20songs\x20mp3/mp4✔️','artist','1696864LhRNaG','#youtubemp3\x20','Click\x20and\x20see\x20search\x20results➡️','1903650hbfehs','botName'];_0x2e80=function(){return _0xa89407;};return _0x2e80();}function _0x273d(_0x4ef19f,_0x243995){const _0x2e80aa=_0x2e80();return _0x273d=function(_0x273de0,_0x9d809e){_0x273de0=_0x273de0-0x8a;let _0x339c83=_0x2e80aa[_0x273de0];return _0x339c83;},_0x273d(_0x4ef19f,_0x243995);}const _0x5b3fb0=_0x273d;(function(_0x3197f1,_0xbae31d){const _0x4fcbd6=_0x273d,_0x3f780e=_0x3197f1();while(!![]){try{const _0x2bda7d=parseInt(_0x4fcbd6(0xa0))/0x1*(parseInt(_0x4fcbd6(0x98))/0x2)+parseInt(_0x4fcbd6(0x96))/0x3+parseInt(_0x4fcbd6(0xaa))/0x4+parseInt(_0x4fcbd6(0x99))/0x5*(parseInt(_0x4fcbd6(0xa6))/0x6)+-parseInt(_0x4fcbd6(0xad))/0x7+-parseInt(_0x4fcbd6(0x9a))/0x8+-parseInt(_0x4fcbd6(0x90))/0x9*(parseInt(_0x4fcbd6(0x95))/0xa);if(_0x2bda7d===_0xbae31d)break;else _0x3f780e['push'](_0x3f780e['shift']());}catch(_0x25f037){_0x3f780e['push'](_0x3f780e['shift']());}}}(_0x2e80,0x3be32));{if(q[_0x5b3fb0(0x93)](_0x5b3fb0(0x8f)))return reply(examquery);if(args[_0x5b3fb0(0xa5)]<0x1)return m[_0x5b3fb0(0xa3)](from,_0x5b3fb0(0x9c),{'quoted':m});const fresh=await searchResult(args[_0x5b3fb0(0x9d)]('\x20'));console[_0x5b3fb0(0x9e)](fresh);let sections=[];for(let i=0x0;i<fresh[_0x5b3fb0(0xa5)];i++){const list={'title':i+0x1+'.\x20'+fresh[i][_0x5b3fb0(0x94)],'rows':[{'title':'[\x20🎵\x20]\x20MP3','rowId':_0x5b3fb0(0xab)+fresh[i][_0x5b3fb0(0xa7)],'description':_0x5b3fb0(0xa1)+fresh[i][_0x5b3fb0(0xa9)]+_0x5b3fb0(0x9f)+fresh[i][_0x5b3fb0(0x97)]+_0x5b3fb0(0x8c)+fresh[i]['duration']['label']+_0x5b3fb0(0xa2)+(fresh[i][_0x5b3fb0(0x92)]?'YouTube\x20Music':'YouTube')+'\x0a\x0aℹ️Id\x20:\x20'+fresh[i]['id']},{'title':_0x5b3fb0(0x91),'rowId':_0x5b3fb0(0x8d)+fresh[i]['url'],'description':_0x5b3fb0(0xa1)+fresh[i]['artist']+_0x5b3fb0(0x9f)+fresh[i]['album']+_0x5b3fb0(0x8c)+fresh[i]['duration'][_0x5b3fb0(0x8b)]+_0x5b3fb0(0xa2)+(fresh[i][_0x5b3fb0(0x92)]?'YouTube\x20Music':_0x5b3fb0(0x8e))+_0x5b3fb0(0x9b)+fresh[i]['id']}]};sections['push'](list);}const sendm=sock['sendMessage'](from,{'text':_0x5b3fb0(0xa8),'footer':Options['info'][_0x5b3fb0(0x8a)],'title':_0x5b3fb0(0xa4),'buttonText':_0x5b3fb0(0xac),'sections':sections},{'quoted':m});}
  break
//soon :v males -specstor
   /* case 'plray': case 'lagu': case 'musik': case 'yts': case 'ytsearch':
   if (q.includes('--help')) return reply(examquery) 
if (isLimit < 1) return reply("Limit kamu sudah habis , silahkan beli dengan cara #buy limit _jumlah_")
kurangLimit(m.sender, 1)
reply(`Satu limit terpakai \nSisa limit kamu : ${getLimit(m.sender)}`) 

   let zaz = await fetchJson(`https:/\/\api-specstor.herokuapp.com/api/download/ytsearch?query=${q}&apikey=Specstor`)
  
    let buttons = [
    {
       buttonId: `${prefix}ytmp4 https:/\/\youtu.be/${zaz.result.id}`, 
       buttonText: {
        displayText: 'MP4'
      }, type: 1},
      {
       buttonId: `${prefix}ytmp3 https:/\/\youtu.be/${zaz.result.id}`, 
       buttonText: {
        displayText: 'MP3'
      }, type: 1},
    ]
    let buttonMessage = {
      image: { url: zaz.result.thumbnail },
      caption: zaz.result.title,
      footer: Options.info.botName,
      buttons: buttons,
      headerType: 4
     }
...
if (q.includes('--help')) return reply(examkosong) 
  ...
  break
//fixed by specstor nobob yang sebelumnya
case 'emojimix': {
if (q.includes('--help')) return reply('Example : .emojimix 😳+😔') 
reply('Tunggu Sebentar')
    ...
    let buttonMessage = {
      image: { url: res.url },
      caption: "Nih \n\n𝙼𝚊𝚞 𝙹𝚊𝚍𝚒i𝚗 𝚂𝚝𝚒𝚌𝚔𝚎𝚛?\n𝚃𝚒𝚗𝚐𝚐𝚊𝚕 𝚁𝚎𝚙𝚕𝚢 𝙿𝚎𝚜𝚊𝚗 𝙸𝚗𝚒 𝙿𝚊𝚔𝚎 𝙲𝚘𝚖𝚖𝚊𝚗𝚍 .𝚜",
      footer: 'Mau Jadiin Sticker?\nTinggal Reply Pesan Ini Pake Command .s\n\n'+Options.info.botName,
      headerType: 4
     }
     sock.sendMessage(from, buttonMessage, { quoted: m })
     }
...
if (q.includes('--help')) return reply(examkosong) 
  ...
  kurangLimit(m.sender, 1)
  reply(`Satu limit terpakai \nSisa limit kamu : ${getLimit(m.sender)}`)
   let za = await fetchJson(`https:/\/\api.waifu.pics/nsfw/waifu`)
 ...
 break
//let specs = await fetchJson(`https:/\/\api-specstor.herokuapp.com/api/nsfw/${q}?apikey=Specstor`)
case 'nsfwwaifu': case 'nsfwwf':{
if (q.includes('--help')) return reply(examkosong) 
  if (!isInventoryLimit){ addInventoriLimit(m.sender) }
  if (isLimit < 1) return reply("Limit kamu sudah habis , silahkan beli dengan cara #buy limit _jumlah_")
  kurangLimit(m.sender, 1)
 ...
 break 
case 'nsfwneko': case 'nsfwnk':{
if (q.includes('--help')) return reply(examkosong) 
  if (!isInventoryLimit){ addInventoriLimit(m.sender) }
  if (isLimit < 1) return reply("Limit kamu sudah habis , silahkan beli dengan cara #buy limit _jumlah_")
  kurangLimit(m.sender, 1)
  reply(`Satu limit terpakai \nSisa limit kamu : ${getLimit(m.sender)}`)
   let za = await fetchJson(`https:/\/\api.waifu.pics/nsfw/neko`)
 ...
 break 
 case 'nsfwbj': case 'nsfwblowjob':{
if (q.includes('--help')) return reply(examkosong) 
  if (!isInventoryLimit){ addInventoriLimit(m.sender) }
  if (isLimit < 1) return reply("Limit kamu sudah habis , silahkan beli dengan cara #buy limit _jumlah_")
  kurangLimit(m.sender, 1)
  reply(`Satu limit terpakai \nSisa limit kamu : ${getLimit(m.sender)}`)
   let specs = await fetchJson(`https:/\/\api-specstor.herokuapp.com/api/nsfw/blowjob?apikey=Specstor`)
 ...
 break
case 'nsfwht': case 'nsfwhentai':{
if (q.includes('--help')) return reply(examkosong) 
  if (!isInventoryLimit){ addInventoriLimit(m.sender) }
  if (isLimit < 1) return reply("Limit kamu sudah habis , silahkan beli dengan cara #buy limit _jumlah_")
  kurangLimit(m.sender, 1)
  reply(`Satu limit terpakai \nSisa limit kamu : ${getLimit(m.sender)}`)
   let specs = await fetchJson(`https:/\/\api-specstor.herokuapp.com/api/nsfw/hentai?apikey=Specstor`)
 ...
 break
case 'nsfwcm': case 'nsfwcum':{
if (q.includes('--help')) return reply(examkosong) 
  if (!isInventoryLimit){ addInventoriLimit(m.sender) }
  if (isLimit < 1) return reply("Limit kamu sudah habis , silahkan beli dengan cara #buy limit _jumlah_")
  kurangLimit(m.sender, 1)
  reply(`Satu limit terpakai \nSisa limit kamu : ${getLimit(m.sender)}`)
riapi.nekos("cum_jpg").then(url => {
let buttonMessage = {
      image: { url: url },
      caption: "Nih",
      footer: 'Mau Jadiin Sticker?\nTinggal Reply Pesan Ini Pake Command .s\n\n'+Options.info.botName,
...
break
case 'nsfwal': case 'nsfwanal':{
if (q.includes('--help')) return reply(examkosong) 
  if (!isInventoryLimit){ addInventoriLimit(m.sender) }
  if (isLimit < 1) return reply("Limit kamu sudah habis , silahkan beli dengan cara #buy limit _jumlah_")
  kurangLimit(m.sender, 1)
  reply(`Satu limit terpakai \nSisa limit kamu : ${getLimit(m.sender)}`)
riapi.nekos("anal").then(url => {
let buttonMessage = {
      image: { url: url },
      caption: "Nih",
      footer: 'Mau Jadiin Sticker?\nTinggal Reply Pesan Ini Pake Command .s\n\n'+Options.info.botName,
...
break
case 'nsfwfm': case 'nsfwfemdom':{
if (q.includes('--help')) return reply(examkosong) 
  if (!isInventoryLimit){ addInventoriLimit(m.sender) }
  if (isLimit < 1) return reply("Limit kamu sudah habis , silahkan beli dengan cara #buy limit _jumlah_")
  kurangLimit(m.sender, 1)
  reply(`Satu limit terpakai \nSisa limit kamu : ${getLimit(m.sender)}`)
   let specs = await fetchJson(`https:/\/\api-specstor.herokuapp.com/api/nsfw/femdom?apikey=Specstor`)
 ...
 break
case 'nsfwoy': case 'nsfworgy':{
if (q.includes('--help')) return reply(examkosong) 
  if (!isInventoryLimit){ addInventoriLimit(m.sender) }
  if (isLimit < 1) return reply("Limit kamu sudah habis , silahkan beli dengan cara #buy limit _jumlah_")
  kurangLimit(m.sender, 1)
  reply(`Satu limit terpakai \nSisa limit kamu : ${getLimit(m.sender)}`)
   let specs = await fetchJson(`https:/\/\api-specstor.herokuapp.com/api/nsfw/orgy?apikey=Specstor`)
 ...
 break
case 'nsfwgif': case 'nsfwhentaigif': case 'nsfwhtgif':
  if (q.includes('--help')) return reply(examkosong) 
if (isLimit < 1) return reply("Limit kamu sudah habis , silahkan beli dengan cara #buy limit _jumlah_")
kurangLimit(m.sender, 1)
reply(`Satu limit terpakai \nSisa limit kamu : ${getLimit(m.sender)}`) 
const Speccs = fs.readFileSync("./FunctionMD/scrape/Result/RandomVideo/nsfwanime.json");
const Specss = JSON.parse(Speccs)
const Speczs = Math.floor(Math.random() * Specss.length)
const Speczstor = Specss[Speczs]
   let aman = `Aman Ngab Ngak Ada Suara\n\n`
   let buttonszx = [{
       index: 1, 
       urlButton: {
       displayText: 'Portofolio Web', 
   ...
   break
//end nsfw by specstor
case 'translate':
let specs = await fetchJson(`https:/\/\specstor-api.herokuapp.com/api/translate?kata=${q}&apikey=uwu`)

let artinya = `Artinya : `

reply(artinya + specs.result.text)
break
case 'emojitopng':
let specsss = await fetchJson(`https:/\/\specstor-api.herokuapp.com/api/maker/emoji2png?apikey=uwu&text=${q}`)
...
break
 ...
 let zas = await fetchJson(`https:/\/\api-specstor.herokuapp.com/api/nsfw/hentaigif?apikey=Specstor`)
